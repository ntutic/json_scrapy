...
- Execution engine (``jscrapy.core.engine.jscrapyengine``)
- Execution manager (``jscrapy.core.manager.jscrapymanager``)
- Extension manager (``jscrapy.extension.extensions``)
- Spider manager (``jscrapy.spider.spiders``)
- Stats collector (``jscrapy.stats.stats``)
- Logging system (``jscrapy.log``)
- Signals system (``jscrapy.xlib.pydispatcher``)
...
- **crawler**: ``jscrapy.crawler.Crawler`` instance (replaces current
  ``jscrapy.core.manager.ExecutionManager``) - instantiated with a ``Settings``
  object

   - **crawler.settings**: ``jscrapy.conf.Settings`` instance (passed in the ``__init__`` method)
   - **crawler.extensions**: ``jscrapy.extension.ExtensionManager`` instance
   - **crawler.engine**: ``jscrapy.core.engine.ExecutionEngine`` instance
   ...
   - **crawler.log**: Logger class with methods replacing the current
     ``jscrapy.log`` functions. Logging would be started (if enabled) on
...
::
   ...
   #!python
   from jscrapy.core.exceptions import NotConfigured
   from jscrapy.conf import settings
...
``jscrapy.command.cmdline`` module will:

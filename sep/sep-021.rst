...
* simple to manage: adding or removing extensions should be just a matter of
  adding or removing lines in a ``jscrapy.cfg`` file
...
Add-ons are defined in the ``jscrapy.cfg`` file, inside the ``[addons]``
section.

To enable the "httpcache" addon, either shipped with Scrapy or in the Python
search path, create an entry for it in your ``jscrapy.cfg``, like this::
...
Examples::
    ...
    def addon_configure(settings):
        settings.overrides['DOWNLADER_MIDDLEWARES'].update({
            'jscrapy.contrib.downloadermiddleware.httpcache.HttpCacheMiddleware': 900,
        })

...
from jscrapy.settings import default_settings
...
def get_settings_priority(priority):
    ...
    :attr:`~jscrapy.settings.SETTINGS_PRIORITIES` dictionary and returns its
...
class BaseSettings(MutableMapping):
    ...
    already an instance of :class:`~jscrapy.settings.BaseSettings`, in which
    case the existing priority levels will be kept).  If the ``priority``
    argument is a string, the priority name will be looked up in
    :attr:`~jscrapy.settings.SETTINGS_PRIORITIES`. Otherwise, a specific integer
    should be provided.

    Once the object is created, new settings can be loaded or updated with the
    :meth:`~jscrapy.settings.BaseSettings.set` method, and can be accessed with
    the square bracket notation of dictionaries, or with the
    :meth:`~jscrapy.settings.BaseSettings.get` method of the instance and its
    ...
    def getdict(self, name, default=None):
        ...
        :class:`~jscrapy.settings.BaseSettings` instance itself, it will be
        converted to a dictionary, containing all its current settings values
        as they would be returned by :meth:`~jscrapy.settings.BaseSettings.get`,
    ...
    def maxpriority(self):
        ...
        :attr:`~jscrapy.settings.SETTINGS_PRIORITIES` if there are no settings
        stored.
        """
        if len(self) > 0:
    ...
    def set(self, name, value, priority='project'):
        ...
        (through the :meth:`~jscrapy.crawler.Crawler.configure` method),
        ...
        :param priority: the priority of the setting. Should be a key of
            :attr:`~jscrapy.settings.SETTINGS_PRIORITIES` or an integer
    ...
    def setmodule(self, module, priority='project'):
        ...
        :meth:`~jscrapy.settings.BaseSettings.set` for every globally declared
    ...
    def update(self, values, priority='project'):
        ...
        This is a helper function that calls
        :meth:`~jscrapy.settings.BaseSettings.set` for every item of ``values``
        with the provided ``priority``.

        If ``values`` is a string, it is assumed to be JSON-encoded and parsed
        into a dict with ``json.loads()`` first. If it is a
        :class:`~jscrapy.settings.BaseSettings` instance, the per-key priorities
        will be used and the ``priority`` parameter ignored. This allows
        inserting/updating settings with different priorities with a single
        command.

        :param values: the settings names and values
        :type values: dict or string or :class:`~jscrapy.settings.BaseSettings`
...
class Settings(BaseSettings):
    ...
    :class:`~jscrapy.settings.BaseSettings`. Additionally, after instantiation

...
from jscrapy.exceptions import ScrapyDeprecationWarning, UsageError

from jscrapy.settings import BaseSettings
from jscrapy.utils.deprecate import update_classpath
from jscrapy.utils.python import without_none_values
...
def closest_jscrapy_cfg(path='.', prevpath=None):
    """Return the path to the closest jscrapy.cfg file by traversing the current
    directory and its parents
    """
    if path == prevpath:
        return ''
    path = os.path.abspath(path)
    cfgfile = os.path.join(path, 'jscrapy.cfg')
    if os.path.exists(cfgfile):
        return cfgfile
    return closest_jscrapy_cfg(os.path.dirname(path), path)
...
def get_sources(use_closest=True):
    ...
    sources = [
        '/etc/jscrapy.cfg',
        r'c:\jscrapy\jscrapy.cfg',
        xdg_config_home + '/jscrapy.cfg',
        os.path.expanduser('~/.jscrapy.cfg'),
    ]
    if use_closest:
        sources.append(closest_jscrapy_cfg())
    return sources

...
-   When :class:`~jscrapy.downloadermiddlewares.httpproxy.HttpProxyMiddleware`
    processes a request with :reqmeta:`proxy` metadata, and that
    :reqmeta:`proxy` metadata includes proxy credentials,
    :class:`~jscrapy.downloadermiddlewares.httpproxy.HttpProxyMiddleware` sets
    ...
    :class:`~jscrapy.downloadermiddlewares.httpproxy.HttpProxyMiddleware` and
    ...
    :class:`~jscrapy.downloadermiddlewares.httpproxy.HttpProxyMiddleware` is now
...
-   :class:`~jscrapy.crawler.CrawlerProcess` supports again crawling multiple
...
-   The ``jscrapy parse -h`` command no longer throws an error (:issue:`5481`,
    :issue:`5482`)
...
-   When a :class:`~jscrapy.http.Request` object with cookies defined gets a
    redirect response causing a new :class:`~jscrapy.http.Request` object to be
    scheduled, the cookies defined in the original
    :class:`~jscrapy.http.Request` object are no longer copied into the new
    :class:`~jscrapy.http.Request` object.

    If you manually set the ``Cookie`` header on a
    :class:`~jscrapy.http.Request` object and the domain name of the redirect
    URL is not an exact match for the domain of the URL of the original
    :class:`~jscrapy.http.Request` object, your ``Cookie`` header is now dropped
    from the new :class:`~jscrapy.http.Request` object.
    ...
    .. _cjvr-mfj7-j4j8 security advisory: https://github.com/jscrapy/jscrapy/security/advisories/GHSA-cjvr-mfj7-j4j8
    ...
    .. note:: It is still possible to enable the sharing of cookies between
              ...
              :class:`~jscrapy.http.Request` class for more information.

-   When the domain of a cookie, either received in the ``Set-Cookie`` header
    of a response or defined in a :class:`~jscrapy.http.Request` object, is set
    ...
    .. _mfjm-vh54-3f96 security advisory: https://github.com/jscrapy/jscrapy/security/advisories/GHSA-mfjm-vh54-3f96
...
-   The ``formdata`` parameter of :class:`~jscrapy.FormRequest`, if specified
...
-   In :class:`jscrapy.core.engine.ExecutionEngine`, methods
    :meth:`~jscrapy.core.engine.ExecutionEngine.crawl`,
    :meth:`~jscrapy.core.engine.ExecutionEngine.download`,
    :meth:`~jscrapy.core.engine.ExecutionEngine.schedule`,
    and :meth:`~jscrapy.core.engine.ExecutionEngine.spider_is_idle`
    now raise :exc:`RuntimeError` if called before
    :meth:`~jscrapy.core.engine.ExecutionEngine.open_spider`. (:issue:`5090`)

    These methods used to assume that
    :attr:`ExecutionEngine.slot <jscrapy.core.engine.ExecutionEngine.slot>` had
    been defined by a prior call to
    :meth:`~jscrapy.core.engine.ExecutionEngine.open_spider`, so they were
...
-   The ``_encoding`` field of serialized :class:`~jscrapy.http.Request` objects
...
-   ``jscrapy.http.TextResponse.body_as_unicode``, deprecated in Scrapy 2.2, has
    now been removed. (:issue:`5393`)

-   ``jscrapy.item.BaseItem``, deprecated in Scrapy 2.2, has now been removed.
    (:issue:`5398`)

-   ``jscrapy.item.DictItem``, deprecated in Scrapy 1.8, has now been removed.
    (:issue:`5398`)

-   ``jscrapy.Spider.make_requests_from_url``, deprecated in Scrapy 1.4, has now
...
-   :mod:`jscrapy.utils.reqser` is deprecated. (:issue:`5130`)

    -   Instead of :func:`~jscrapy.utils.reqser.request_to_dict`, use the new
        :meth:`Request.to_dict <jscrapy.http.Request.to_dict>` method.

    -   Instead of :func:`~jscrapy.utils.reqser.request_from_dict`, use the new
        :func:`jscrapy.utils.request.request_from_dict` function.

-   In :mod:`jscrapy.squeues`, the following queue classes are deprecated:
    :class:`~jscrapy.squeues.PickleFifoDiskQueueNonRequest`,
    :class:`~jscrapy.squeues.PickleLifoDiskQueueNonRequest`,
    :class:`~jscrapy.squeues.MarshalFifoDiskQueueNonRequest`,
    and :class:`~jscrapy.squeues.MarshalLifoDiskQueueNonRequest`. You should
    instead use:
    :class:`~jscrapy.squeues.PickleFifoDiskQueue`,
    :class:`~jscrapy.squeues.PickleLifoDiskQueue`,
    :class:`~jscrapy.squeues.MarshalFifoDiskQueue`,
    and :class:`~jscrapy.squeues.MarshalLifoDiskQueue`. (:issue:`5117`)

-   Many aspects of :class:`jscrapy.core.engine.ExecutionEngine` that come from
    a time when this class could handle multiple :class:`~jscrapy.Spider`
    objects at a time have been deprecated. (:issue:`5090`)

    -   The :meth:`~jscrapy.core.engine.ExecutionEngine.has_capacity` method
        is deprecated.

    -   The :meth:`~jscrapy.core.engine.ExecutionEngine.schedule` method is
        deprecated, use :meth:`~jscrapy.core.engine.ExecutionEngine.crawl` or
        :meth:`~jscrapy.core.engine.ExecutionEngine.download` instead.

    -   The :attr:`~jscrapy.core.engine.ExecutionEngine.open_spiders` attribute
        is deprecated, use :attr:`~jscrapy.core.engine.ExecutionEngine.spider`
        instead.

    -   The ``spider`` parameter is deprecated for the following methods:

        -   :meth:`~jscrapy.core.engine.ExecutionEngine.spider_is_idle`

        -   :meth:`~jscrapy.core.engine.ExecutionEngine.crawl`

        -   :meth:`~jscrapy.core.engine.ExecutionEngine.download`

        Instead, call :meth:`~jscrapy.core.engine.ExecutionEngine.open_spider`
        first to set the :class:`~jscrapy.Spider` object.
...
-   :mod:`jscrapy.utils.defer` gained 2 new functions,
    :func:`~jscrapy.utils.defer.deferred_to_future` and
    :func:`~jscrapy.utils.defer.maybe_deferred_to_future`, to help :ref:`await
...
-   :attr:`Request.cookies <jscrapy.Request.cookies>` values that are
    :class:`bool`, :class:`float` or :class:`int` are cast to :class:`str`.
    (:issue:`5252`, :issue:`5253`)

-   You may now raise :exc:`~jscrapy.exceptions.CloseSpider` from a handler of
    the :signal:`spider_idle` signal to customize the reason why the spider is
    stopping. (:issue:`5191`)

-   When using
    :class:`~jscrapy.downloadermiddlewares.httpproxy.HttpProxyMiddleware`, the
...
-   :class:`~jscrapy.http.Request` and :class:`~jscrapy.http.Response` now have
    an ``attributes`` attribute that makes subclassing easier. For
    :class:`~jscrapy.http.Request`, it also allows subclasses to work with
    :func:`jscrapy.utils.request.request_from_dict`. (:issue:`1877`,
    :issue:`5130`, :issue:`5218`)

-   The :meth:`~jscrapy.core.scheduler.BaseScheduler.open` and
    :meth:`~jscrapy.core.scheduler.BaseScheduler.close` methods of the
    :ref:`scheduler <topics-scheduler>` are now optional. (:issue:`3559`)

-   HTTP/1.1 :exc:`~jscrapy.core.downloader.handlers.http11.TunnelError`
...
-   :class:`~jscrapy.loader.ItemLoader` now supports non-text responses.
...
-   The :setting:`TWISTED_REACTOR` and :setting:`ASYNCIO_EVENT_LOOP` settings
    are no longer ignored if defined in :attr:`~jscrapy.Spider.custom_settings`.
...
-   :attr:`Request.cb_kwargs <jscrapy.Request.cb_kwargs>` once again allows the
    ``callback`` keyword. (:issue:`5237`, :issue:`5251`, :issue:`5264`)

-   Made :func:`jscrapy.utils.response.open_in_browser` support more complex
    HTML. (:issue:`5319`, :issue:`5320`)

-   Fixed :attr:`CSVFeedSpider.quotechar
    <jscrapy.spiders.CSVFeedSpider.quotechar>` being interpreted as the CSV file
...
-   :class:`LinkExtractor <jscrapy.linkextractors.lxmlhtml.LxmlLinkExtractor>`
...
-   :ref:`run-multiple-spiders` now features
    :func:`jscrapy.utils.project.get_project_settings` usage. (:issue:`5070`)
...
-   Extended the documentation of the
    :class:`~jscrapy.extensions.statsmailer.StatsMailer` extension.
    (:issue:`5199`, :issue:`5217`)

-   Added :setting:`JOBDIR` to :ref:`topics-settings`. (:issue:`5173`,
    :issue:`5224`)

-   Documented :attr:`Spider.attribute <jscrapy.Spider.attribute>`.
    (:issue:`5174`, :issue:`5244`)

-   Documented :attr:`TextResponse.urljoin <jscrapy.http.TextResponse.urljoin>`.
    (:issue:`1582`)

-   Added the ``body_length`` parameter to the documented signature of the
    :signal:`headers_received` signal. (:issue:`5270`)

-   Clarified :meth:`SelectorList.get <jscrapy.selector.SelectorList.get>` usage
...
-   Significantly reduced memory usage by
    :func:`jscrapy.utils.response.response_httprepr`, used by the
    :class:`~jscrapy.downloadermiddlewares.stats.DownloaderStats` downloader
...
*   **Security bug fix:**
    ...
    If you use
    :class:`~jscrapy.downloadermiddlewares.httpauth.HttpAuthMiddleware`
    ...
    Finally, if you are a user of `jscrapy-splash`_, know that this version of
    Scrapy breaks compatibility with jscrapy-splash 0.7.2 and earlier. You will
    need to upgrade jscrapy-splash to a greater version for it to continue to
    work.
...
-   New :func:`~jscrapy.downloadermiddlewares.retry.get_retry_request` function
    to retry requests from spider callbacks

-   New :class:`~jscrapy.signals.headers_received` signal that allows stopping
    downloads early

-   New :class:`Response.protocol <jscrapy.http.Response.protocol>` attribute
...
-   The :mod:`jscrapy.utils.py36` module is now deprecated in favor of
    :mod:`jscrapy.utils.asyncgen`. (:issue:`4900`)
...
-   The new :func:`jscrapy.downloadermiddlewares.retry.get_retry_request`
    function may be used from spider callbacks or middlewares to handle the
    retrying of a request beyond the scenarios that
    :class:`~jscrapy.downloadermiddlewares.retry.RetryMiddleware` supports.
    (:issue:`3590`, :issue:`3685`, :issue:`4902`)

-   The new :class:`~jscrapy.signals.headers_received` signal gives early access
    to response headers and allows :ref:`stopping downloads
    <topics-stop-response-download>`.
    (:issue:`1772`, :issue:`4897`)

-   The new :attr:`Response.protocol <jscrapy.http.Response.protocol>`
...
-   :ref:`Stats <topics-stats>` now include the following entries that indicate
    ...
    :class:`~jscrapy.extensions.feedexport.FileFeedStorage` or
    :class:`~jscrapy.extensions.feedexport.FTPFeedStorage`.

    (:issue:`3947`, :issue:`4850`)

-   The :class:`~jscrapy.spidermiddlewares.urllength.UrlLengthMiddleware` spider
...
-   The
    :class:`~jscrapy.downloadermiddlewares.httpcompression.HttpCompressionMiddleware`
...
-   Covered jscrapy-bench_ in :ref:`benchmarking`.
...
-   Fixed the IPv6-capable DNS resolver
    :class:`~jscrapy.resolver.CachingHostnameResolver` for download handlers
    that call
...
*   :class:`~jscrapy.downloadermiddlewares.cookies.CookiesMiddleware` once again
    discards cookies defined in :attr:`Request.headers
    <jscrapy.http.Request.headers>`.
    ...
    <jscrapy.http.Request>` parameter.
...
*   :class:`jscrapy.extensions.feedexport.S3FeedStorage` no longer reads the
    ...
    <jscrapy.extensions.feedexport.S3FeedStorage.from_crawler>`
    (:issue:`4356`, :issue:`4411`, :issue:`4688`)

*   :attr:`Rule.process_request <jscrapy.spiders.crawl.Rule.process_request>`
...
*   The :class:`jscrapy.utils.python.WeakKeyCache` class is now deprecated
    (:issue:`4684`, :issue:`4701`)

*   The :func:`jscrapy.utils.boto.is_botocore` function is now deprecated, use
    :func:`jscrapy.utils.boto.is_botocore_available` instead (:issue:`4734`,
    :issue:`4776`)
...
*   The following methods of :ref:`media pipelines <topics-media-pipeline>` now
    ...
    -   In :class:`jscrapy.pipelines.files.FilesPipeline`:

        -   :meth:`~jscrapy.pipelines.files.FilesPipeline.file_downloaded`

        -   :meth:`~jscrapy.pipelines.files.FilesPipeline.file_path`

        -   :meth:`~jscrapy.pipelines.files.FilesPipeline.media_downloaded`

        -   :meth:`~jscrapy.pipelines.files.FilesPipeline.media_to_download`

    -   In :class:`jscrapy.pipelines.images.ImagesPipeline`:

        -   :meth:`~jscrapy.pipelines.images.ImagesPipeline.file_downloaded`

        -   :meth:`~jscrapy.pipelines.images.ImagesPipeline.file_path`

        -   :meth:`~jscrapy.pipelines.images.ImagesPipeline.get_images`

        -   :meth:`~jscrapy.pipelines.images.ImagesPipeline.image_downloaded`

        -   :meth:`~jscrapy.pipelines.images.ImagesPipeline.media_downloaded`

        -   :meth:`~jscrapy.pipelines.images.ImagesPipeline.media_to_download`
...
*   :ref:`Downloader middlewares <topics-downloader-middleware>` can now
    override :class:`response.request <jscrapy.http.Response.request>`.

    If a :ref:`downloader middleware <topics-downloader-middleware>` returns
    a :class:`~jscrapy.http.Response` object from
    ...
    with a custom :class:`~jscrapy.http.Request` object assigned to
    :class:`response.request <jscrapy.http.Response.request>`:

    -   The response is handled by the callback of that custom
        :class:`~jscrapy.http.Request` object, instead of being handled by the
        callback of the original :class:`~jscrapy.http.Request` object

    -   That custom :class:`~jscrapy.http.Request` object is now sent as the
        ``request`` argument to the :signal:`response_received` signal, instead
        of the original :class:`~jscrapy.http.Request` object
...
*   The ``__init__`` method of :class:`~jscrapy.exporters.CsvItemExporter` now
...
*   The :class:`~jscrapy.downloadermiddlewares.httpproxy.HttpProxyMiddleware`
...
*   :func:`jscrapy.utils.iterators.xmliter` now supports namespaced node names
    (:issue:`861`, :issue:`4746`)

*   :class:`~jscrapy.Request` objects can now have ``about:`` URLs, which can
...
*   Improved the documentation of
    ...
    :class:`~jscrapy.link.Link` class (:issue:`4751`, :issue:`4775`)

*   Clarified the impact of :setting:`CONCURRENT_REQUESTS` when using the
    :class:`~jscrapy.extensions.closespider.CloseSpider` extension
    (:issue:`4836`)
...
*   The ``jscrapy.utils.python.retry_on_eintr`` function is now deprecated
    (:issue:`4683`)
...
*   :meth:`Request.from_curl <jscrapy.http.Request.from_curl>` and
    :func:`~jscrapy.utils.curl.curl_to_request_kwargs` now also support
    ``--data-raw`` (:issue:`4612`)

*   A ``parse`` callback may now be used in built-in spider subclasses, such
    as :class:`~jscrapy.spiders.CrawlSpider` (:issue:`712`, :issue:`732`,
...
*   :meth:`Request.from_curl <jscrapy.http.Request.from_curl>` and
    :func:`~jscrapy.utils.curl.curl_to_request_kwargs` now set the request
...
* :ref:`dataclass objects <dataclass-items>` and
  ...
  <item-types>`
* New :meth:`TextResponse.json <jscrapy.http.TextResponse.json>` method
* New :signal:`bytes_received` signal that allows canceling response download
* :class:`~jscrapy.downloadermiddlewares.cookies.CookiesMiddleware` fixes
...
*   :meth:`TextResponse.body_as_unicode
    <jscrapy.http.TextResponse.body_as_unicode>` is now deprecated, use
    :attr:`TextResponse.text <jscrapy.http.TextResponse.text>` instead
    (:issue:`4546`, :issue:`4555`, :issue:`4579`)

*   :class:`jscrapy.item.BaseItem` is now deprecated, use
    :class:`jscrapy.item.Item` instead (:issue:`4534`)
...
*   A new :meth:`TextResponse.json <jscrapy.http.TextResponse.json>` method
...
*   The dictionaries in the result list of a :ref:`media pipeline
    ...
    <jscrapy.pipelines.files.FilesPipeline.get_media_requests>` for more
...
*   :ref:`Link extractors <topics-link-extractors>` are now serializable,
    ...
    <jscrapy.http.Request.cb_kwargs>` or
    :attr:`Request.meta <jscrapy.http.Request.meta>` when :ref:`persisting
...
*   :func:`jscrapy.utils.misc.create_instance` now raises a :exc:`TypeError`
    exception if the resulting instance is ``None`` (:issue:`4528`,
    :issue:`4532`)

.. _itemadapter: https://github.com/jscrapy/itemadapter


Bug fixes
~~~~~~~~~

*   :class:`~jscrapy.downloadermiddlewares.cookies.CookiesMiddleware` no longer
    discards cookies defined in :attr:`Request.headers
    <jscrapy.http.Request.headers>` (:issue:`1992`, :issue:`2400`)

*   :class:`~jscrapy.downloadermiddlewares.cookies.CookiesMiddleware` no longer
    re-encodes cookies defined as :class:`bytes` in the ``cookies`` parameter
    of the ``__init__`` method of :class:`~jscrapy.http.Request`
...
*   :class:`~jscrapy.spiders.Spider` callbacks defined using :doc:`coroutine
    syntax <topics/coroutines>` no longer need to return an iterable, and may
    instead return a :class:`~jscrapy.http.Request` object, an
...
*   Fix a :exc:`KeyError` exception being sometimes raised from
    :class:`jscrapy.utils.datatypes.LocalWeakReferencedCache` (:issue:`4597`,
    :issue:`4599`)
...
*   Removed a misleading import line from the
    :func:`jscrapy.utils.log.configure_logging` code example (:issue:`4510`,
    :issue:`4587`)
...
*   The display-on-hover behavior of internal documentation references now also
    ...
    <jscrapy.http.Request.meta>` keys, :ref:`settings <topics-settings>` and
...
*   Improved :class:`~jscrapy.spiderloader.SpiderLoader` test coverage for
...
* New :attr:`Response.ip_address <jscrapy.http.Response.ip_address>` attribute
...
*   The :class:`~jscrapy.downloadermiddlewares.chunked.ChunkedTransferMiddleware`
    middleware has been removed, including the entire
    :class:`jscrapy.downloadermiddlewares.chunked` module; chunked transfers
    work out of the box (:issue:`4431`)

*   The ``spiders`` property has been removed from
    :class:`~jscrapy.crawler.Crawler`, use :class:`CrawlerRunner.spider_loader
    <jscrapy.crawler.CrawlerRunner.spider_loader>` or instantiate
    :setting:`SPIDER_LOADER_CLASS` with your settings instead (:issue:`4398`)

*   The ``MultiValueDict``, ``MultiValueDictKeyError``, and ``SiteNode``
    classes have been removed from :mod:`jscrapy.utils.datatypes`
    (:issue:`4400`)
...
*   The new :attr:`Response.ip_address <jscrapy.http.Response.ip_address>`
...
*   A warning is now issued when a value in
    :attr:`~jscrapy.spiders.Spider.allowed_domains` includes a port
...
*   ``None`` values in :attr:`~jscrapy.spiders.Spider.allowed_domains` no longer
...
*   Covered the `curl2jscrapy <https://michael-shub.github.io/curl2jscrapy/>`_
...
*   :meth:`Response.follow_all <jscrapy.http.Response.follow_all>` now supports
...
* New :meth:`Response.follow_all <jscrapy.http.Response.follow_all>` method
* :ref:`FTP support <media-pipeline-ftp>` for media pipelines
* New :attr:`Response.certificate <jscrapy.http.Response.certificate>`
  attribute
...
*   The ``__init__`` method of custom download handlers (see
    ...
    *   :class:`jscrapy.core.downloader.handlers.datauri.DataURIDownloadHandler`

    *   :class:`jscrapy.core.downloader.handlers.file.FileDownloadHandler`
...
*   We have refactored the :class:`jscrapy.core.scheduler.Scheduler` class and
...
*   The :ref:`Scrapy shell <topics-shell>` no longer provides a `sel` proxy
    object, use :meth:`response.selector <jscrapy.http.Response.selector>`
    instead (:issue:`4347`)

*   LevelDB support has been removed (:issue:`4112`)

*   The following functions have been removed from :mod:`jscrapy.utils.python`:
...
*   :class:`jscrapy.linkextractors.FilteringLinkExtractor` is deprecated, use
    :class:`jscrapy.linkextractors.LinkExtractor
    <jscrapy.linkextractors.lxmlhtml.LxmlLinkExtractor>` instead (:issue:`4045`)

*   The ``noconnect`` query string argument of proxy URLs is deprecated and
    should be removed from proxy URLs (:issue:`4198`)

*   The :meth:`next <jscrapy.utils.python.MutableChain.next>` method of
    :class:`jscrapy.utils.python.MutableChain` is deprecated, use the global
    :func:`next` function or :meth:`MutableChain.__next__
    <jscrapy.utils.python.MutableChain.__next__>` instead (:issue:`4153`)
...
*   The new :meth:`Response.follow_all <jscrapy.http.Response.follow_all>`
    method offers the same functionality as
    :meth:`Response.follow <jscrapy.http.Response.follow>` but supports an
...
*   The new :attr:`Response.certificate <jscrapy.http.Response.certificate>`
...
*   The new :attr:`Response.cb_kwargs <jscrapy.http.Response.cb_kwargs>`
    attribute serves as a shortcut for :attr:`Response.request.cb_kwargs
    <jscrapy.http.Request.cb_kwargs>` (:issue:`4331`)

*   :meth:`Response.follow <jscrapy.http.Response.follow>` now supports a
    ``flags`` parameter, for consistency with :class:`~jscrapy.http.Request`
...
*   :class:`~jscrapy.spiders.Rule` now accepts an ``errback`` parameter
    (:issue:`4000`)

*   :class:`~jscrapy.http.Request` no longer requires a ``callback`` parameter
    when an ``errback`` parameter is specified (:issue:`3586`, :issue:`4008`)

*   :class:`~jscrapy.logformatter.LogFormatter` now supports some additional
    methods:

    *   :class:`~jscrapy.logformatter.LogFormatter.download_error` for
        download errors

    *   :class:`~jscrapy.logformatter.LogFormatter.item_error` for exceptions
        raised during item processing by :ref:`item pipelines
        <topics-item-pipeline>`

    *   :class:`~jscrapy.logformatter.LogFormatter.spider_error` for exceptions
...
*   :class:`~jscrapy.spiders.Spider` objects now raise an :exc:`AttributeError`
    exception if they do not have a :class:`~jscrapy.spiders.Spider.start_urls`
    attribute nor reimplement :class:`~jscrapy.spiders.Spider.start_requests`,
    but have a ``start_url`` attribute (:issue:`4133`, :issue:`4170`)

*   :class:`~jscrapy.exporters.BaseItemExporter` subclasses may now use
...
*   A new ``keep_fragments`` parameter of
    ``jscrapy.utils.request.request_fingerprint`` allows to generate
...
*   :class:`jscrapy.utils.python.MutableChain.__iter__` now returns ``self``,
...
*   :class:`LinkExtractor.extract_links
    <jscrapy.linkextractors.lxmlhtml.LxmlLinkExtractor.extract_links>` no longer
...
*   :class:`~jscrapy.http.Request` no longer accepts strings as ``url`` simply
    because they have a colon (:issue:`2552`, :issue:`4094`)

*   The correct encoding is now used for attach names in
    :class:`~jscrapy.mail.MailSender` (:issue:`4229`, :issue:`4239`)

*   :class:`~jscrapy.dupefilters.RFPDupeFilter`, the default
...
*   Adding items to a :class:`jscrapy.utils.datatypes.LocalCache` object
...
*   Fixed a typo in the message of the :exc:`ValueError` exception raised when
    :func:`jscrapy.utils.misc.create_instance` gets both ``settings`` and
...
*   Improved the documentation about :meth:`LinkExtractor.extract_links
    <jscrapy.linkextractors.lxmlhtml.LxmlLinkExtractor.extract_links>` and
    simplified :ref:`topics-link-extractors` (:issue:`4045`)

*   Clarified how :class:`ItemLoader.item <jscrapy.loader.ItemLoader.item>`
    works (:issue:`3574`, :issue:`4099`)

*   Clarified that :func:`logging.basicConfig` should not be used when also
    using :class:`~jscrapy.crawler.CrawlerProcess` (:issue:`2149`,
    :issue:`2352`, :issue:`3146`, :issue:`3960`)

*   Clarified the requirements for :class:`~jscrapy.http.Request` objects
...
*   Covered a backward-incompatible change in Scrapy 1.7.0 affecting custom
    :class:`jscrapy.core.scheduler.Scheduler` subclasses (:issue:`4274`)
...
*   The ``push`` method no longer receives a second positional parameter
    ...
    the new :meth:`~jscrapy.core.scheduler.ScrapyPriorityQueue.priority`
    method in :class:`jscrapy.core.scheduler.ScrapyPriorityQueue`
    subclasses.
...
*   In the ``__init__`` method or the ``from_crawler`` or ``from_settings``
    ...
    *   The ``serialize`` parameter is no longer passed. The disk queue
        ...
        :func:`~jscrapy.utils.reqser.request_to_dict` and
        :func:`~jscrapy.utils.reqser.request_from_dict` functions from the
        :mod:`jscrapy.utils.reqser` module.
...
:class:`~jscrapy.core.scheduler.ScrapyPriorityQueue` and
:class:`~jscrapy.core.scheduler.DownloaderAwarePriorityQueue` classes from
:mod:`jscrapy.core.scheduler` and may affect subclasses:
...
*   In the ``__init__`` method, most of the changes described above apply.
    ...
    *   ``downstream_queue_cls``, which replaced ``qfactory``, must be
        ...
        the new
        :meth:`ScrapyPriorityQueue.qfactory <jscrapy.core.scheduler.ScrapyPriorityQueue.qfactory>`
        or
        :meth:`DownloaderAwarePriorityQueue.pqfactory <jscrapy.core.scheduler.DownloaderAwarePriorityQueue.pqfactory>`
        methods.
...
*   The following class attributes have been added:
    ...
    *   :attr:`~jscrapy.core.scheduler.ScrapyPriorityQueue.crawler`

    *   :attr:`~jscrapy.core.scheduler.ScrapyPriorityQueue.downstream_queue_cls`
        (details above)

    *   :attr:`~jscrapy.core.scheduler.ScrapyPriorityQueue.key` (details above)

*   The ``serialize`` attribute has been removed (details above)

The following changes affect specifically the
:class:`~jscrapy.core.scheduler.ScrapyPriorityQueue` class and may affect
subclasses:

*   A new :meth:`~jscrapy.core.scheduler.ScrapyPriorityQueue.priority`
    method has been added which, given a request, returns
    ``request.priority * -1``.

    It is used in :meth:`~jscrapy.core.scheduler.ScrapyPriorityQueue.push`
    to make up for the removal of its ``priority`` parameter.

*   The ``spider`` attribute has been removed. Use
    :attr:`crawler.spider <jscrapy.core.scheduler.ScrapyPriorityQueue.crawler>`
    instead.

The following changes affect specifically the
:class:`~jscrapy.core.scheduler.DownloaderAwarePriorityQueue` class and may
affect subclasses:

*   A new :attr:`~jscrapy.core.scheduler.DownloaderAwarePriorityQueue.pqueues`
    attribute offers a mapping of downloader slot names to the
    corresponding instances of
    :attr:`~jscrapy.core.scheduler.DownloaderAwarePriorityQueue.downstream_queue_cls`.
...
* Dropped Python 3.4 support and updated minimum requirements; made Python 3.8
  support official
* New :meth:`Request.from_curl <jscrapy.http.Request.from_curl>` class method
...
*   ``JSONRequest`` is now called :class:`~jscrapy.http.JsonRequest` for
...
*   :class:`~jscrapy.loader.ItemLoader` now turns the values of its input item
    into lists:
...
*   A new :meth:`Request.from_curl <jscrapy.http.Request.from_curl>` class
...
*   :class:`~jscrapy.spiders.Rule` no longer requires a :class:`LinkExtractor
    <jscrapy.linkextractors.lxmlhtml.LxmlLinkExtractor>` parameter
...
*   Callbacks that receive keyword arguments
    (see :attr:`Request.cb_kwargs <jscrapy.http.Request.cb_kwargs>`) can now be
    tested using the new :class:`@cb_kwargs
    <jscrapy.contracts.default.CallbackKeywordArgumentsContract>`
    :ref:`spider contract <topics-contracts>` (:issue:`3985`, :issue:`3988`)

*   When a :class:`@scrapes <jscrapy.contracts.default.ScrapesContract>` spider
...
*   :meth:`ItemLoader.load_item() <jscrapy.loader.ItemLoader.load_item>` no
    longer makes later calls to :meth:`ItemLoader.get_output_value()
    <jscrapy.loader.ItemLoader.get_output_value>` or
    :meth:`ItemLoader.load_item() <jscrapy.loader.ItemLoader.load_item>` return
    empty data (:issue:`3804`, :issue:`3819`, :issue:`3897`, :issue:`3976`,
    :issue:`3998`, :issue:`4036`)

*   Fixed :class:`~jscrapy.statscollectors.DummyStatsCollector` raising a
    :exc:`TypeError` exception (:issue:`4007`, :issue:`4052`)

*   :meth:`FilesPipeline.file_path
    <jscrapy.pipelines.files.FilesPipeline.file_path>` and
    :meth:`ImagesPipeline.file_path
    <jscrapy.pipelines.images.ImagesPipeline.file_path>` no longer choose
...
*   A memory-handling and error-handling issue in
    :func:`jscrapy.utils.ssl.get_temp_key_info` has been fixed (:issue:`3920`)
...
*   API documentation added for :class:`~jscrapy.exporters.MarshalItemExporter`
    and :class:`~jscrapy.exporters.PythonItemExporter` (:issue:`3973`)

*   API documentation added for :class:`~jscrapy.item.BaseItem` and
    :class:`~jscrapy.item.ItemMeta` (:issue:`3999`)
...
*   ``jscrapy.xlib`` has been removed (:issue:`4015`)
...
*   The LevelDB_ storage backend
    (``jscrapy.extensions.httpcache.LeveldbCacheStorage``) of
    :class:`~jscrapy.downloadermiddlewares.httpcache.HttpCacheMiddleware` is
...
*   ``jscrapy.item.DictItem`` is deprecated, use :class:`~jscrapy.item.Item`
    instead (:issue:`3999`)
...
:meth:`ItemLoader.load_item() <jscrapy.loader.ItemLoader.load_item>` once again
makes later calls to :meth:`ItemLoader.get_output_value()
<jscrapy.loader.ItemLoader.get_output_value>` or :meth:`ItemLoader.load_item()
<jscrapy.loader.ItemLoader.load_item>` return empty data.
...
*   :class:`~jscrapy.crawler.Crawler`,
    :class:`CrawlerRunner.crawl <jscrapy.crawler.CrawlerRunner.crawl>` and
    :class:`CrawlerRunner.create_crawler <jscrapy.crawler.CrawlerRunner.create_crawler>`
    no longer accept a :class:`~jscrapy.spiders.Spider` subclass instance, they
    only accept a :class:`~jscrapy.spiders.Spider` subclass now.

    :class:`~jscrapy.spiders.Spider` subclass instances were never meant to
    work, and they were not working as one would expect: instead of using the
    passed :class:`~jscrapy.spiders.Spider` subclass instance, their
    :class:`~jscrapy.spiders.Spider.from_crawler` method was called to generate
    a new instance.
...
*   Non-default values for the :setting:`SCHEDULER_PRIORITY_QUEUE` setting
    ...
    :class:`~jscrapy.http.Request` objects instead of arbitrary Python data
    structures.

*   An additional ``crawler`` parameter has been added to the ``__init__``
    method of the :class:`~jscrapy.core.scheduler.Scheduler` class. Custom
...
*   A new scheduler priority queue,
    ``jscrapy.pqueues.DownloaderAwarePriorityQueue``, may be
...
*   A new :attr:`Request.cb_kwargs <jscrapy.http.Request.cb_kwargs>` attribute
    provides a cleaner way to pass keyword arguments to callback methods
    (:issue:`1138`, :issue:`3563`)

*   A new :class:`JSONRequest <jscrapy.http.JsonRequest>` class offers a more
    convenient way to build JSON requests (:issue:`3504`, :issue:`3505`)

*   A ``process_request`` callback passed to the :class:`~jscrapy.spiders.Rule`
    ``__init__`` method now receives the :class:`~jscrapy.http.Response` object that
    originated the request as its second argument (:issue:`3682`)

*   A new ``restrict_text`` parameter for the
    :attr:`LinkExtractor <jscrapy.linkextractors.lxmlhtml.LxmlLinkExtractor>`
...
*   A new :meth:`Item.deepcopy() <jscrapy.item.Item.deepcopy>` method makes it
    easier to :ref:`deep-copy items <copying-items>` (:issue:`1493`,
    :issue:`3671`)

*   :class:`~jscrapy.extensions.corestats.CoreStats` also logs
    ``elapsed_time_seconds`` now (:issue:`3638`)

*   Exceptions from :class:`~jscrapy.loader.ItemLoader` :ref:`input and output
...
*   :class:`~jscrapy.crawler.Crawler`,
    :class:`CrawlerRunner.crawl <jscrapy.crawler.CrawlerRunner.crawl>` and
    :class:`CrawlerRunner.create_crawler <jscrapy.crawler.CrawlerRunner.create_crawler>`
    now fail gracefully if they receive a :class:`~jscrapy.spiders.Spider`
...
*   :meth:`~jscrapy.spidermiddlewares.SpiderMiddleware.process_spider_exception`
...
*   The images pipeline (:class:`~jscrapy.pipelines.images.ImagesPipeline`) no
...
*   Fixed a memory leak in ``jscrapy.pipelines.media.MediaPipeline`` affecting,
...
*   :meth:`FormRequest.from_response() <jscrapy.http.FormRequest.from_response>`
...
*   The documentation of :class:`~jscrapy.spiders.Rule` now covers how to access
    the text of a link when using :class:`~jscrapy.spiders.CrawlSpider`
...
*   The :setting:`LOGSTATS_INTERVAL` setting (:issue:`3730`), the
    :meth:`FilesPipeline.file_path <jscrapy.pipelines.files.FilesPipeline.file_path>`
    and
    :meth:`ImagesPipeline.file_path <jscrapy.pipelines.images.ImagesPipeline.file_path>`
    methods (:issue:`2253`, :issue:`3609`) and the
    :meth:`Crawler.stop() <jscrapy.crawler.Crawler.stop>` method (:issue:`3842`)
    are now documented
...
*   ``jscrapy.conf`` (use :attr:`Crawler.settings
    <jscrapy.crawler.Crawler.settings>`)

*   From ``jscrapy.core.downloader.handlers``:

    *   ``http.HttpDownloadHandler`` (use ``http10.HTTP10DownloadHandler``)

*   ``jscrapy.loader.ItemLoader._get_values`` (use ``_get_xpathvalues``)

*   ``jscrapy.loader.XPathItemLoader`` (use :class:`~jscrapy.loader.ItemLoader`)

*   ``jscrapy.log`` (see :ref:`topics-logging`)

*   From ``jscrapy.pipelines``:
...
*   From both ``jscrapy.selector`` and ``jscrapy.selector.lxmlsel``:

    *   ``HtmlXPathSelector`` (use :class:`~jscrapy.selector.Selector`)

    *   ``XmlXPathSelector`` (use :class:`~jscrapy.selector.Selector`)

    *   ``XPathSelector`` (use :class:`~jscrapy.selector.Selector`)

    *   ``XPathSelectorList`` (use :class:`~jscrapy.selector.Selector`)

*   From ``jscrapy.selector.csstranslator``:
...
*   From :class:`~jscrapy.selector.Selector`:
...
*   From :class:`~jscrapy.selector.SelectorList`:
...
*   ``jscrapy.spiders.BaseSpider`` (use :class:`~jscrapy.spiders.Spider`)

*   From :class:`~jscrapy.spiders.Spider` (and subclasses):

    *   ``DOWNLOAD_DELAY`` (use :ref:`download_delay
        <spider-download_delay-attribute>`)

    *   ``set_crawler`` (use :meth:`~jscrapy.spiders.Spider.from_crawler`)

*   ``jscrapy.spiders.spiders`` (use :class:`~jscrapy.spiderloader.SpiderLoader`)

*   ``jscrapy.telnet`` (use :mod:`jscrapy.extensions.telnet`)

*   From ``jscrapy.utils.python``:

    *   ``str_to_unicode`` (use ``to_unicode``)

    *   ``unicode_to_str`` (use ``to_bytes``)

*   ``jscrapy.utils.response.body_or_str``
...
*   The ``queuelib.PriorityQueue`` value for the
    ...
    ``jscrapy.pqueues.ScrapyPriorityQueue`` instead.

*   ``process_request`` callbacks passed to :class:`~jscrapy.spiders.Rule` that
    do not accept two arguments are deprecated.

*   The following modules are deprecated:

    *   ``jscrapy.utils.http`` (use `w3lib.http`_)

    *   ``jscrapy.utils.markup`` (use `w3lib.html`_)

    *   ``jscrapy.utils.multipart`` (use `urllib3`_)

*   The ``jscrapy.utils.datatypes.MergeDict`` class is deprecated for Python 3
    code bases. Use :class:`~collections.ChainMap` instead. (:issue:`3878`)

*   The ``jscrapy.utils.gz.is_gzipped`` function is deprecated. Use
    ``jscrapy.utils.gz.gzip_magic_number`` instead.
...
* ``jscrapy.contracts`` fixes and new features;
...
``jscrapy.contracts`` improvements
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
* ``jscrapy shell --help`` mentions syntax required for local files
...
* ``jscrapy.command``
* ``jscrapy.contrib`` (with all submodules)
* ``jscrapy.contrib_exp`` (with all submodules)
* ``jscrapy.dupefilter``
* ``jscrapy.linkextractor``
* ``jscrapy.project``
* ``jscrapy.spider``
* ``jscrapy.spidermanager``
* ``jscrapy.squeue``
* ``jscrapy.stats``
* ``jscrapy.statscol``
* ``jscrapy.utils.decorator``
...
* Deprecated jscrapy.interfaces.ISpiderManager is removed; please use
  jscrapy.interfaces.ISpiderLoader.
...
* ``jscrapy.http.cookies.CookieJar.clear`` accepts "domain", "path" and "name"
...
* Warnings, exception and logging messages are improved to make debugging
  easier.
* ``jscrapy parse`` command now allows to set custom request meta via
  ``--meta`` argument.
...
* Default Scrapy User-Agent now uses https link to jscrapy.org (:issue:`2983`).
...
* Logging of settings overridden by ``custom_settings`` is fixed;
  **this is technically backward-incompatible** because the logger
  changes from ``[jscrapy.utils.log]`` to ``[jscrapy.crawler]``. If you're
...
- Google Cloud Storage support for FilesPipeline and ImagesPipeline
  (:issue:`2923`).
- New ``--meta`` option of the "jscrapy parse" command allows to pass additional
...
- ``jscrapy.mail.MailSender`` now works in Python 3 (it requires Twisted 17.9.0)
...
- Fix logging of settings overridden by ``custom_settings``;
  **this is technically backward-incompatible** because the logger
  changes from ``[jscrapy.utils.log]`` to ``[jscrapy.crawler]``, so please
...
There's a new :meth:`response.follow <jscrapy.http.TextResponse.follow>` method
...
``jscrapy.Request`` objects directly:
...
For example, instead of this::
    ...
    for href in response.css('li.page a::attr(href)').extract():
        ...
        yield jscrapy.Request(url, self.parse, encoding=response.encoding)
...
- Default to ``canonicalize=False`` in
  :class:`jscrapy.linkextractors.LinkExtractor
  <jscrapy.linkextractors.lxmlhtml.LxmlLinkExtractor>`
...
- Added ``flags`` argument and attribute to :class:`Request <jscrapy.http.Request>`
  objects (:issue:`2047`)
- Support Anonymous FTP (:issue:`2342`)
- Added ``retry/count``, ``retry/max_reached`` and ``retry/reason_count/<reason>``
  stats to :class:`RetryMiddleware <jscrapy.downloadermiddlewares.retry.RetryMiddleware>`
  (:issue:`2543`)
- Added ``httperror/response_ignored_count`` and ``httperror/response_ignored_status_count/<status>``
  stats to :class:`HttpErrorMiddleware <jscrapy.spidermiddlewares.httperror.HttpErrorMiddleware>`
  (:issue:`2566`)
- Customizable :setting:`Referrer policy <REFERRER_POLICY>` in
  :class:`RefererMiddleware <jscrapy.spidermiddlewares.referer.RefererMiddleware>`
  (:issue:`2306`)
...
- ``jscrapy.utils.datatypes.CaselessDict`` now accepts ``Mapping`` instances and
  not only dicts (:issue:`2646`)
- :ref:`Media downloads <topics-media-pipeline>`, with
  :class:`~jscrapy.pipelines.files.FilesPipeline` or
  :class:`~jscrapy.pipelines.images.ImagesPipeline`, can now optionally handle
...
- Per-request retry times with the new :reqmeta:`max_retry_times` meta key
  (:issue:`2642`)
- ``python -m jscrapy`` as a more explicit alternative to ``jscrapy`` command
  (:issue:`2740`)
...
- Properly handle whitespaces in action attribute in
  :class:`~jscrapy.http.FormRequest` (:issue:`2548`)
...
- Always decompress ``Content-Encoding: gzip`` at :class:`HttpCompressionMiddleware
  <jscrapy.downloadermiddlewares.httpcompression.HttpCompressionMiddleware>` stage (:issue:`2391`)
...
- Catch more specific ``os.error`` exception in
  ``jscrapy.pipelines.files.FSFilesStore`` (:issue:`2644`)
...
- Mention issue with :meth:`FormRequest.from_response
  <jscrapy.http.FormRequest.from_response>` due to bug in lxml (:issue:`2572`)
...
- Remove ``__nonzero__`` from :class:`~jscrapy.selector.SelectorList`
  docs (:issue:`2683`)
...
- Support ``'True'`` and ``'False'`` string values for boolean settings (:issue:`2519`);
  you can now do something like ``jscrapy crawl myspider -s REDIRECT_ENABLED=False``.
...
- Fix ``jscrapy.Item`` inheritance on Python 3.6 (:issue:`2511`).
...
some backported code from Twisted (``jscrapy.xlib.tx.*``),
...
- ``jscrapy fetch url``, ``jscrapy shell url`` and ``fetch(url)`` inside
...
- By default, logger names now use a long-form path, e.g. ``[jscrapy.extensions.logstats]``,
  instead of the shorter "top-level" variant of prior releases (e.g. ``[jscrapy]``);
...
- As a consequence, we got rid of ``jscrapy.xlib.tx.*`` modules, which
...
- Add more dependencies' version info in ``jscrapy version`` verbose output (:issue:`2404`)
- Remove all ``*.pyc`` files from source distribution (:issue:`2386`)

.. _conda-forge: https://anaconda.org/conda-forge/jscrapy
...
- ``jscrapy`` is imported by default when using shell tools (:command:`shell`,
...
- HTTP cache extension and plugins that use the ``.jscrapy`` data directory now
...
- ``Selector`` does not allow passing both ``response`` and ``text`` anymore
  (:issue:`2153`).
- Fixed logging of wrong callback name with ``jscrapy parse`` (:issue:`2169`).
- Fix for an odd gzip decompression bug (:issue:`1606`).
- Fix for selected callbacks when using ``CrawlSpider`` with :command:`jscrapy parse <parse>`
  (:issue:`2225`).
...
- These bug fixes may require your attention:
  ...
  - Fix shell files argument handling (:issue:`1710`, :issue:`1550`).
    If you try ``jscrapy shell index.html`` it will try to load the URL http://index.html,
    use ``jscrapy shell ./index.html`` to load a local file.
  ...
  - Exporters now work on unicode, instead of bytes by default (:issue:`1080`).
    If you use :class:`~jscrapy.exporters.PythonItemExporter`, you may want to
...
<https://github.com/jscrapy/jscrapy/wiki/Python-3-Porting>`_. As a result, now
...
.. _`Code of Conduct`: https://github.com/jscrapy/jscrapy/blob/master/CODE_OF_CONDUCT.md
...
- The following datatypes were deprecated (:issue:`1720`):
  ...
  + ``jscrapy.utils.datatypes.MultiValueDictKeyError``
  + ``jscrapy.utils.datatypes.MultiValueDict``
  + ``jscrapy.utils.datatypes.SiteNode``

- The previously bundled ``jscrapy.xlib.pydispatch`` library was deprecated and
...
- ``telnetconsole`` was relocated to ``extensions/`` (:issue:`1524`).
  ...
  + Note: telnet is not enabled on Python 3
    (https://github.com/jscrapy/jscrapy/pull/1524#issuecomment-146985595)

.. _parsel: https://github.com/jscrapy/parsel
...
- Fixed ``jscrapy.http.FormRequest.from_response`` when there is a ``<base>``
  tag (:issue:`1564`).
...
- Fixed :class:`~jscrapy.exporters.PythonItemExporter` and CSVExporter for
...
- don't run tests twice on Travis if a PR is made from a jscrapy/jscrapy branch (:commit:`a83ab41`)
...
- minor: jscrapy.Spider docs grammar (:commit:`1ddcc7b`)
...
- minor: jscrapy.Spider grammar and clarity (:commit:`9c9d2e0`)
...
Start using it by defining a :attr:`~jscrapy.spiders.Spider.custom_settings`
...
:meth:`~jscrapy.spiders.Spider.log` method you’ll have access to a custom
:attr:`~jscrapy.spiders.Spider.logger` created for the spider to issue log
events:
...
new projects and dissolving both ``jscrapy.contrib`` and ``jscrapy.contrib_exp``
...
+=====================================+=====================================+
| jscrapy.commands.deploy              | `jscrapyd-client <https://github.com |
|                                     | /jscrapy/jscrapyd-client>`_           |
|                                     | (See other alternatives here:       |
|                                     | :ref:`topics-deploy`)               |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib.djangoitem           | `jscrapy-djangoitem <https://github. |
|                                     | com/jscrapy-plugins/jscrapy-djangoite |
|                                     | m>`_                                |
+-------------------------------------+-------------------------------------+
| jscrapy.webservice                   | `jscrapy-jsonrpc <https://github.com |
|                                     | /jscrapy-plugins/jscrapy-jsonrpc>`_   |
+-------------------------------------+-------------------------------------+

``jscrapy.contrib_exp`` and ``jscrapy.contrib`` dissolutions

+-------------------------------------+-------------------------------------+
| Old location                        | New location                        |
+=====================================+=====================================+
| jscrapy.contrib\_exp.downloadermidd\ | jscrapy.downloadermiddlewares.decom\ |
| leware.decompression                | pression                            |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib\_exp.iterators       | jscrapy.utils.iterators              |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib.downloadermiddleware | jscrapy.downloadermiddlewares        |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib.exporter             | jscrapy.exporters                    |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib.linkextractors       | jscrapy.linkextractors               |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib.loader               | jscrapy.loader                       |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib.loader.processor     | jscrapy.loader.processors            |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib.pipeline             | jscrapy.pipelines                    |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib.spidermiddleware     | jscrapy.spidermiddlewares            |
+-------------------------------------+-------------------------------------+
| jscrapy.contrib.spiders              | jscrapy.spiders                      |
+-------------------------------------+-------------------------------------+
| * jscrapy.contrib.closespider        | jscrapy.extensions.\*                |
| * jscrapy.contrib.corestats          |                                     |
| * jscrapy.contrib.debug              |                                     |
| * jscrapy.contrib.feedexport         |                                     |
| * jscrapy.contrib.httpcache          |                                     |
| * jscrapy.contrib.logstats           |                                     |
| * jscrapy.contrib.memdebug           |                                     |
| * jscrapy.contrib.memusage           |                                     |
| * jscrapy.contrib.spiderstate        |                                     |
| * jscrapy.contrib.statsmailer        |                                     |
| * jscrapy.contrib.throttle           |                                     |
+-------------------------------------+-------------------------------------+
...
| Old location                        | New location                        |
+=====================================+=====================================+
| jscrapy.command                      | jscrapy.commands                     |
+-------------------------------------+-------------------------------------+
| jscrapy.dupefilter                   | jscrapy.dupefilters                  |
+-------------------------------------+-------------------------------------+
| jscrapy.linkextractor                | jscrapy.linkextractors               |
+-------------------------------------+-------------------------------------+
| jscrapy.spider                       | jscrapy.spiders                      |
+-------------------------------------+-------------------------------------+
| jscrapy.squeue                       | jscrapy.squeues                      |
+-------------------------------------+-------------------------------------+
| jscrapy.statscol                     | jscrapy.statscollectors              |
+-------------------------------------+-------------------------------------+
| jscrapy.utils.decorator              | jscrapy.utils.decorators             |
+-------------------------------------+-------------------------------------+
...
| Old location                        | New location                        |
+=====================================+=====================================+
| jscrapy.spidermanager.SpiderManager  | jscrapy.spiderloader.SpiderLoader    |
+-------------------------------------+-------------------------------------+
...
- look in ~/.config/jscrapy.cfg for user config (:issue:`1098`)
...
- remove jscrapy deploy command (:issue:`1102`)
...
- Added deployment section covering jscrapyd-deploy and shub (:issue:`1124`)
...
- jscrapy.utils.misc.load_object should print full traceback (:issue:`902`)
...
- disable jscrapy.telnet if twisted.conch is not available (:issue:`1161`)
...
- remove unnecessary check from jscrapy.utils.spider.iter_spider_output
  (:issue:`1078`)
...
- pem file is used by mockserver and required by jscrapy bench (:commit:`5eddc68`)
- jscrapy bench needs jscrapy.tests* (:commit:`d6cb999`)
...
- jscrapy.bat is not needed anymore (:commit:`c3861cf`)
...
- fix tests to account for https://github.com/jscrapy/w3lib/pull/23 (:commit:`f8d366a`)
...
- Make jscrapy.version_info a tuple of integers (:issue:`681`, :issue:`692`)
...
- Improve ``jscrapy check`` contracts command (:issue:`733`, :issue:`752`)
...
- Sort spider list output of ``jscrapy list`` command (:issue:`742`)
...
- [**Backward incompatible**] Switched HTTPCacheMiddleware backend to filesystem (:issue:`541`)
  To restore old backend set ``HTTPCACHE_STORAGE`` to ``jscrapy.contrib.httpcache.DbmCacheStorage``
- Proxy \https:// urls using CONNECT method (:issue:`392`, :issue:`397`)
- Add a middleware to crawl ajax crawleable pages as defined by google (:issue:`343`)
- Rename jscrapy.spider.BaseSpider to jscrapy.spider.Spider (:issue:`510`, :issue:`519`)
...
- jscrapy.mail.MailSender now can connect over TLS or upgrade using STARTTLS (:issue:`327`)
...
- Backport ``jscrapy check`` command fixes and backward compatible multi
...
- Bugfix ``jscrapy.utils.gz.gunzip`` broken by changes in python 2.7.4 (:commit:`4dc76e`)
...
- Simplify internal ``jscrapy.core.scraper`` slot handling (:issue:`271`)
...
- Added ``--pdb`` option to ``jscrapy`` command line tool
- Added :meth:`XPathSelector.remove_namespaces <jscrapy.selector.Selector.remove_namespaces>` which allows to remove all namespaces from XML documents for convenience (to work with namespace-less XPaths). Documented in :ref:`topics-selectors`.
...
- better backward compatibility for jscrapy.conf.settings (:commit:`3403089`)
...
- added :meth:`~jscrapy.spidermiddlewares.SpiderMiddleware.process_start_requests` method to spider middlewares
...
- removed modules: ``jscrapy.xlib.BeautifulSoup`` and ``jscrapy.xlib.ClientForm``
...
- dropped support for Python 2.5. See https://blog.scrapinghub.com/2012/02/27/jscrapy-0-15-dropping-support-for-python-2-5/
- dropped support for Twisted 2.5
- added :setting:`REFERER_ENABLED` setting, to control referer middleware
- changed default user agent to: ``Scrapy/VERSION (+http://jscrapy.org)``
- removed (undocumented) ``HTMLImageLinkExtractor`` class from ``jscrapy.contrib.linkextractors.image``
...
- replaced memory usage acounting with (more portable) `resource`_ module, removed ``jscrapy.utils.memory`` module
- removed signal: ``jscrapy.mail.mail_sent``
...
- removed ``jscrapy.log.started`` attribute

Scrapy 0.14.4
-------------

- added precise to supported Ubuntu distros (:commit:`b7e46df`)
- fixed bug in json-rpc webservice reported in https://groups.google.com/forum/#!topic/jscrapy-users/qgVBmFybNAQ/discussion. also removed no longer supported 'run' command from extras/jscrapy-ws.py (:commit:`340fbdb`)
...
- jscrapyd: fixed documentation link (:commit:`2b4e4c3`)
...
- fixed formatting of jscrapyd doc (:commit:`8bf19e6`)
...
- added ``-o`` option to ``jscrapy crawl``, a shortcut for dumping scraped items into a file (or standard output using ``-``)
...
- Moved spider queues to jscrapyd: ``jscrapy.spiderqueue`` -> ``jscrapyd.spiderqueue`` (:rev:`2708`)
- Moved sqlite utils to jscrapyd: ``jscrapy.utils.sqlite`` -> ``jscrapyd.sqlite`` (:rev:`2781`)
...
- Added new command to edit spiders: ``jscrapy edit`` (:rev:`2636`) and ``-e`` flag to ``genspider`` command that uses it (:rev:`2653`)
...
- Reduced Scrapy codebase by striping part of Scrapy code into two new libraries:
   - `w3lib`_ (several functions from ``jscrapy.utils.{http,markup,multipart,response,url}``, done in :rev:`2584`)
   - `scrapely`_ (was ``jscrapy.contrib.ibl``, done in :rev:`2586`)
- Removed unused function: ``jscrapy.utils.request.request_info()`` (:rev:`2577`)
- Removed googledir project from ``examples/googledir``. There's now a new example project called ``dirbot`` available on GitHub: https://github.com/jscrapy/dirbot
...
- Removed support for passing urls to ``jscrapy crawl`` command (use ``jscrapy parse`` instead) (:rev:`2704`)
- Removed deprecated Execution Queue (:rev:`2704`)
- Removed (undocumented) spider context extension (from jscrapy.contrib.spidercontext) (:rev:`2780`)
- removed ``CONCURRENT_SPIDERS`` setting (use jscrapyd maxproc instead) (:rev:`2789`)
...
- Passed item is now sent in the ``item`` argument of the :signal:`item_passed
  <item_scraped>` (#273)
- Added verbose option to ``jscrapy version`` command, useful for bug reports (#298)
...
- Added ``-c`` argument to ``jscrapy shell`` command
...
- There is now a ``jscrapy server`` command to start a Scrapyd server of the current project
...
- New Scrapy service called ``jscrapyd`` for deploying Scrapy crawlers in production (#218) (documentation available)
...
- New ``jscrapy`` command which replaces the old ``jscrapy-ctl.py`` (#199)
  - there is only one global ``jscrapy`` command now, instead of one ``jscrapy-ctl.py`` per project
  - Added ``jscrapy.bat`` script for running more conveniently from Windows
...
- Removed ``UrlFilterMiddleware`` from ``jscrapy.contrib`` (already disabled by default)
...
- Changes to Scrapy Manager (now called "Crawler"):
   - ``jscrapy.core.manager.ScrapyManager`` class renamed to ``jscrapy.crawler.Crawler``
   - ``jscrapy.core.manager.jscrapymanager`` singleton moved to ``jscrapy.project.crawler``
- Moved module: ``jscrapy.contrib.spidermanager`` to ``jscrapy.spidermanager``
- Spider Manager singleton moved from ``jscrapy.spider.spiders`` to the ``spiders` attribute of ``jscrapy.project.crawler`` singleton.
- moved Stats Collector classes: (#204)
   - ``jscrapy.stats.collector.StatsCollector`` to ``jscrapy.statscol.StatsCollector``
   - ``jscrapy.stats.collector.SimpledbStatsCollector`` to ``jscrapy.contrib.statscol.SimpledbStatsCollector``
...
- moved ``jscrapy.core.signals`` module to ``jscrapy.signals``
   - backward compatibility kept (with deprecation warning)
- moved ``jscrapy.core.exceptions`` module to ``jscrapy.exceptions``
   - backward compatibility kept (with deprecation warning)
- added ``handles_request()`` class method to ``BaseSpider``
- dropped ``jscrapy.log.exc()`` function (use ``jscrapy.log.err()`` instead)
- dropped ``component`` argument of ``jscrapy.log.msg()`` function
- dropped ``jscrapy.log.log_level`` attribute
...
- Added SMTP-AUTH support to jscrapy.mail
- New settings added: ``MAIL_USER``, ``MAIL_PASS`` (:rev:`2065` | #149)
- Added new jscrapy-ctl view command - To view URL in the browser, as seen by Scrapy (:rev:`2039`)
...
- ``jscrapy.command`` modules relocation (:rev:`2035`, :rev:`2036`, :rev:`2037`)
...
- Moved module: ``jscrapy.management.telnet`` to ``jscrapy.telnet`` (:rev:`2047`)
...
- Changed ``jscrapy.utils.response.get_meta_refresh()`` signature (:rev:`1804`)
- Removed deprecated ``jscrapy.item.ScrapedItem`` class - use ``jscrapy.item.Item instead`` (:rev:`1838`)
- Removed deprecated ``jscrapy.xpath`` module - use ``jscrapy.selector`` instead. (:rev:`1836`)
...
- ``CloseDomain`` extension moved to ``jscrapy.contrib.closespider.CloseSpider`` (:rev:`1833`)
...
- Removed obsolete ``jscrapy.utils.markup.remove_escape_chars`` function - use ``jscrapy.utils.markup.replace_escape_chars`` instead (:rev:`1865`)
...
.. _queuelib: https://github.com/jscrapy/queuelib
...
.. _scrapely: https://github.com/jscrapy/scrapely
...
.. _w3lib: https://github.com/jscrapy/w3lib
.. _w3lib.encoding: https://github.com/jscrapy/w3lib/blob/master/w3lib/encoding.py
